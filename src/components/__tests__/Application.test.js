...
describe("Application", () => {
  ...
  it("shows the save error when failing to save an appointment", async () => {
    ...
    await waitForElement(() => getByText(container, "Archie Cohen"));
    const appointments = getAllByTestId(container, "appointment");
    const appointment = appointments.find(appointment => getByAltText(appointment, "Add"));
    fireEvent.click(getByAltText(appointment, "Add"));
    fireEvent.change(getByPlaceholderText(appointment, "Enter Student Name"), {
      target: { value: "Lydia Miller-Jones" }
    });
    fireEvent.click(getByAltText(appointment, "Sylvia Palmer"));
    fireEvent.click(getByText(appointment, "Save"));
    try {await waitForElement(() => getByText(appointment, "Lydia Miller-Jones"))}
    catch{
      expect(getByText(appointment, "Can't add the appointment.")).toBeInTheDocument();
  ...
  it("shows the delete error when failing to delete an existing appointment", async () => {
    ...
    expect(getByText(appointment, "Are you sure you would like to delete this appointment?")).toBeInTheDocument();
    fireEvent.click(getByText(appointment, "Confirm"));
    try {await waitForElement(() => getByAltText(appointment, "Add"))}
    catch {
      expect(getByText(appointment, "Can't cancel the appointment.")).toBeInTheDocument();
      fireEvent.click(getByAltText(appointment, "Close"));
